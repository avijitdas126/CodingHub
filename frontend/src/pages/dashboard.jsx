...
play()
  ...
  const submit=(e)=>{
    ...
    let play=async()=>{
      try {
        let fetch_data=await fetch('http://localhost:9000/user/code/new/project',{
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
...
if(Number(code)==200){
  ...
  setmodel(false)
  // let url=
  navigator('/editor/0/'+user_detail.name+'/'+input.current.value+'/'+code_id)
    input.current.value=''
  setreload(!reload)
}
else{
  ...
  return (
    ...
    <div className='flex w-full backdrop-filter  backdrop-blur'>
      ...
      <Nav
        ...
        className='w-[85%]'
      />
  <div className='flex gap-2 mt-16 relative flex-col-reverse '>
{codes.map((elem)=>{
  return(
    < Card  user_name={user_detail.name} file_name={elem.file_name.split('.')[0]} ele={elem} created_at={elem.created} file_id={elem.code_id}/>
  )
})}
  ...
  <div className="bg-white p-8 rounded-lg shadow-lg relative">
        ...
        <form>
          <label htmlFor="FileName" className='mb-5'>File Name:</label>
          <div className='flex gap-2 mb-5'>
          <input type="text" placeholder='Enter the File Name' autoFocus={true} className='border-solid border-2  border-black p-2' ref={input}  />
          <div className='border-solid border-2 border-black p-2'><kbd>.html</kbd></div>
          </div>
          <div className="flex gap-5 justify-center">
          <input type="submit" value="Cancel" className="btn bg-slate-600 hover:bg-slate-400 cursor-pointer text-white" onClick={onClose}  />
          <input type="submit" value="Submit" className='btn cursor-pointer' onClick={submit} />
          </div>
