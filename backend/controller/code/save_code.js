...
save_code.post("/", (req, res) => {
  ...
  const play = async () => {
    try {
      ...
      let current_time = moment().format("MMM D, YYYY h:mm A");
      let time = moment(res25?.[0].updated, "MMM D, YYYY h:mm A").unix();
      if (res25.length != 0 && data.userid == res25[0].userid) {
        ...
        let details = await codeDetail.updateOne(
          { code_id },
          {
            $set: {
        ...
        if (res21.length != 0) {
          let dat5 = await Codecss.updateOne(
            { code_id },
            {
              $set: {
                css,
        ...
        if (res22.length != 0) {
          let dat5 = await Codejs.updateOne(
            { code_id },
            {
              $set: {
                js,
